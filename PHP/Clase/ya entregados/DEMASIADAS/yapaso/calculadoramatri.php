...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($cantidad > $unidad) {
            ...
            echo nl2br("<h1>El producto que has seleccionado es: $productoSeleccionado y pillaste una cantidad de: $cantidad $unidad x " . number_format($precio, 2) . " (Por lo que te va a salir la broma a) = " . number_format($subtotal, 2) . "€\n </h1>");

            switch (true) {
                case ($totalSinDescuento > LIMITE_DESCUENTO):
            ...
            $iva = $totalConDescuento * DESCUENTO_ADICIONAL;
            $totalConDescuento -= $descuentoAdicional;

            echo nl2br("Total sin descuento: " . number_format($totalSinDescuento, 2) . "€"."\n");
            ...
            echo nl2br("<p>Promedio de precio por unidad:". number_format($promedioPrecioPorUnidad,2) . "€"."</p>". "\n");

            if ($cantidad % 2 == 0) {
                echo nl2br("<h1>La cantidad de productos es par.</h1>");
            } else {
                echo nl2br("<p>La cantidad de productos es impar.</p> \n");
            }

            if ($totalConIva > 200 || $cantidad > 50) {
                echo nl2br("<p>¡Felicidades! Recibes un producto gratis.</p> \n");
            }
